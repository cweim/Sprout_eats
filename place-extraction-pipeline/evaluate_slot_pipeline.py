#!/usr/bin/env python3
"""Evaluate the slot-based place extraction pipeline against ground truth."""
...
DEFAULT_METADATA = PIPELINE_DIR / "metadata_dataset" / "instagram_metadata_with_media.json"
DEFAULT_GROUND_TRUTH = PIPELINE_DIR / "datasets" / "instagram_ground_truth.json"
DEFAULT_OUTPUT_JSON = PIPELINE_DIR / "datasets" / "slot_pipeline_evaluation.json"
DEFAULT_OUTPUT_MD = PIPELINE_DIR / "datasets" / "slot_pipeline_evaluation.md"
...
def compare_slots_to_ground_truth(slots: list[PlaceEvidence], gt_places: list[dict[str, Any]]) -> dict[str, Any]:
    matched_gt = set()
    matched_slots = set()
...
async def evaluate(
    ...
    *,
    ...
    for record in metadata_records:
        shortcode = record["input"]["shortcode"]
        gt_record = gt_records.get(shortcode, {})
        gt_places = gt_record.get("places", [])
        slots = extract_place_evidence_from_metadata(record)
        slot_comparison = compare_slots_to_ground_truth(slots, gt_places)

        suggestions = []
        suggestion_comparison = None
        if resolve_google and slots:
            resolved = await resolve_place_slots(slots)
            suggestions = suggestions_to_dict(resolved)
            suggestion_comparison = compare_suggestions_to_ground_truth(suggestions, gt_places)
        ...
        evaluations.append({
            ...
            "slots": [asdict(slot) | {"query": slot.query} for slot in slots],
    ...
    summary = {
        ...
        "total_suggested_places": total_suggested if resolve_google else None,
        "total_suggestion_matches": total_suggestion_matches if resolve_google else None,
        "suggestion_precision": (
            round(total_suggestion_matches / total_suggested, 3)
            if resolve_google and total_suggested else None
        ),
        "suggestion_recall": (
            round(total_suggestion_matches / total_expected, 3)
            if resolve_google and total_expected else None
        ),
        "records_with_exact_suggestion_count": (
            records_with_exact_suggestion_count if resolve_google else None
        ),
    }
...
def write_markdown_report(result: dict[str, Any], path: Path) -> None:
    ...
    for record in result["records"]:
        ...
        suggestion_cmp = record.get("suggestion_comparison")
        lines.extend([
            f"### {record['shortcode']}",
            "",
        ...
        if record["suggestions"]:
            lines.extend(["", "Selected suggestions:", ""])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--resolve-google", action="store_true")
...
def main() -> None:
    ...
    result = asyncio.run(
        evaluate(
            args.metadata,
            args.ground_truth,
            resolve_google=args.resolve_google,
        )
    )
